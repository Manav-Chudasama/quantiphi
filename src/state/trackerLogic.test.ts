...
test("adding a meal preserves existing meals and updates totals", () => {
  const firstMeal = createMealItem(
    CUSTOM_ENTRY_PROFILE,
    100,
    "First meal",
    "manual",
  );
  const baseState = buildTrackerState([firstMeal], "maintenance", false);
  const secondMeal = createMealItem(
    CUSTOM_ENTRY_PROFILE,
    200,
    "Second meal",
    "manual",
  ...
  const nextState = applyTrackerAction(baseState, {
    type: "add-meal",
    meal: secondMeal,
  });

  expect(nextState.meals).toHaveLength(2);
  expect(nextState.totals.calories).toBe(
    firstMeal.calories + secondMeal.calories,
  );
});

test("goal switching keeps meals and recalculates budget thresholds", () => {
  const meal = createMealItem(
    MOCK_SCAN_FOODS[0],
    MOCK_SCAN_FOODS[0].suggestedGrams,
    MOCK_SCAN_FOODS[0].label,
    "image",
  );
  ...
  expect(nextState.targets.calories).toBeGreaterThan(
    baseState.targets.calories,
  );
...
test("over-budget additions trigger the warning state and delete actions lower totals", () => {
  ...
  const overState = applyTrackerAction(startingState, {
    type: "add-meal",
    meal,
  });
  ...
  const clearedState = applyTrackerAction(overState, {
    type: "delete-meal",
    id: meal.id,
  });
