...
import { scaleNutrition, sumMacros, type FoodTemplate, type Macros } from '../lib/nutrition'
...
export function buildTrackerState(
  ...
  const validation = totals.calories > targets.calories ? 'over-budget' : 'within-budget'
...
export function applyTrackerAction(state: TrackerState, action: TrackerAction): TrackerState {
  switch (action.type) {
    case 'add-meal': {
      const nextState = buildTrackerState([action.meal, ...state.meals], state.goal, state.warningOpen)

      return {
        ...nextState,
        warningOpen:
          state.validation === 'within-budget' && nextState.validation === 'over-budget'
            ? true
            : state.warningOpen,
      }
    }
    ...
    case 'set-goal': {
      const nextState = buildTrackerState(state.meals, action.goal, state.warningOpen)
